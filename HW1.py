import random

point_hw = input("Введіть пункт домашнього завдання 1-4 >>> ")

if point_hw.isdigit():
    point_hw_int=int(point_hw)
# Home work 1.1
    # 1.Запросіть у користувача ім'я та місячну зарплату в доларах та виведіть їхню річну зарплату в тисячах доларів.
    # Наприклад: «Мішель», «12345» → «Річна зарплата Мішель складає 148 тис. доларів».
    if point_hw_int == 1:
        hw_list1=[input("Введіть ім'я >>> "), int(input("Введіть зарплату >>> "))]
        salary_year = int(hw_list1[1] * 12 / 1000)
        print(f"Річна зарплата {hw_list1[0]} складає {salary_year} тис. доларів")
# Home work 1.2
        # 2. Запросіть ціле число і виведіть True, якщо це парне число діапазоні від 100 до 999, інакше - «False».
    elif point_hw_int == 2:
        int_number = int(input("Введіть ціле число >>> "))
        if 100 < int_number < 999 and int_number % 2 == 0:  
            print("True\n", "Це парне число в діапазоні від 100 до 999") 
        else:
            print("False\n", "Це НЕ парне число в діапазоні від 100 до 999")
        # s = int(input("Введіть трьохзначне число "))
        # print(100 <= s <= 999 and s % 2 == 0)
# Home work 1.3  
        # 3. Як вхідні дані візьмемо ціле число; Це буде ціле число від 101 до 999, а його остання цифра не дорівнює нулю(робити перевірку не обовʼязково) Виведіть число, що складається з чисел першого у зворотньому порядку.
        # Наприклад: 256 → 652.
    elif point_hw_int == 3:
        # number_3= int(input("Введіть ціле число від 101 до 999 (його остання цифра не може бути 0) >>> "))
        # if str(number_3)[-1] != 0 and 101 <= number_3 <= 999:
        #     print(f"Реверс: {str(number_3)[::-1]}")
        # else :
        #     print("Невірно введене число")
        number_3=0
        while number_3 % 10 ==0:
            number_3=random.randint(101, 999)
        print(number_3)
        if number_3 % 10 == 0 and not (100 <= number_3 <= 999):
            print('Число не відповідає параметрам задачі')
        else:
            print(str(number_3)[::-1])
# Home work 1.4
        # 4. Запитайте два цілих числа та виведіть:
        # a. Їхню суму
        # b. Їхня різниця
        # c. результат множення
        # d. Результат поділу першого на друге
        # e. Залишок від поділу першого на друге
        # f. True, якщо перше число більше або дорівнює другому, інакше False.
    elif point_hw_int == 4:
        hw_list2=[int(input("Введіть перше число >>> ")), int(input("Введіть друге число >>> "))]
        print(f"\n Cумма {hw_list2[0]+hw_list2[1]} \n Різниця {hw_list2[0]-hw_list2[1]}\n Результат множення {hw_list2[0]*hw_list2[1]}\n Результат поділу першого на друге {hw_list2[0]/hw_list2[1]}\n Залишок від поділу першого на друге {hw_list2[0]%hw_list2[1]}\n {hw_list2[0]>hw_list2[1]}")
    else:
        print("Номер такого завдання відсутній або це не число від 1 до 4")
else:
    print("Введено не число")

